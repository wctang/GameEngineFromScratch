...
// global declarations
D3D12_VIEWPORT                  g_ViewPort = {0.0f, 0.0f, static_cast<float>(nScreenWidth), static_cast<float>(nScreenHeight)};   // viewport structure
...
ComPtr<ID3D12PipelineState>     g_pPipelineState;                   // an object maintains the state of all currently set shaders and certain fixed function state objects such as the input assembler, tesselator, rasterizer and output manager
...
void InitGraphics() {
  ...
  ThrowIfFailed(g_pVertexBuffer->Map(0, &readRange, reinterpret_cast<void**>(&pVertexDataBegin)));               // map the buffer
...
void CreateGraphicsResources(HWND hWnd)
{
  if (g_pSwapChain.Get() == nullptr) {
    ...
    if (bUseWarpDevice) {
      ...
      ThrowIfFailed(D3D12CreateDevice( warpAdapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&g_pDev)));
    } else {
      ComPtr<IDXGIAdapter1> hardwareAdapter;
      GetHardwareAdapter(factory.Get(), &hardwareAdapter);
      ThrowIfFailed(D3D12CreateDevice( hardwareAdapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&g_pDev)));
    }
